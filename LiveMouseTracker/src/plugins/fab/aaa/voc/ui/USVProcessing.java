...
public class USVProcessing extends PluginActionable implements PluginThreaded, ActionListener , VocAnalysisListener {
	...
	private void processBatch() {
		...
		{
			if ( optionArray[i].equals("-recursive") )
			{
				String basePath = optionArray[i+1];
				System.out.println("Recursive search of wav file starting from " + basePath );
				
				String[] types = {"wav"};
				Collection<File> files2 = FileUtils.listFiles( new File( basePath ), types , true);				
				fileToProcess = new ArrayList<File>( files2 );
			}
		}
