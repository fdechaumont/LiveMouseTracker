...
public class NoiseCanceler {
	...
	private void computeNoiseCancellingWithMagnitude(double[][] magnitude , double[][] magnitudeDenoised )
	{
		...
		boolean REMOVE_VERTICAL_WITH_KERNEL = true;
		boolean REMOVE_STEERING_FILTER = true;

		for ( int x=0; x<width ; x++ )
		{
			verticalSignal = magnitude[x];
			verticalSignalDenoised = magnitudeDenoised[x];
			double mean = MathUtil.mean( verticalSignal );
...
*/
		...
		if ( REMOVE_VERTICAL_WITH_KERNEL )
		{
