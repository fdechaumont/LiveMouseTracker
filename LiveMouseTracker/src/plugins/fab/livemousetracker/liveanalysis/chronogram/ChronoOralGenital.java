...
public class ChronoOralGenital extends ChronoFeatureAbstract implements ProcessAnimalPairFeature {

	public ChronoOralGenital() {
		super ( "Oral-genital Contact" , "Animal A's head is touching Animal B's genitals." , TimeLineDataType.BOOLEAN );
	}

	public void process( Animal animalA , Animal animalB , Integer frameStart, Integer frameEnd )
	{
//		description = animalA.getName()+ "'s head is touching " + animalB.getName()+ "'s genitals.";
			...
			if ( detectionA != null && detectionB != null )
			{
				...
				try
				{
					if ( detectionA.getFrontPoint().toPoint2D().distance(
							detectionB.getBackPoint().toPoint2D() ) < ChronoConstant.MAX_DISTANCE_HEAD_HEAD_GENITAL_THRESHOLD )
					{
				...
				}catch( NullPointerException e)
				{
					...
					//System.err.println("OralGenital: Can't process. Front/Back info not available.");
				}
			}
		}
	}
