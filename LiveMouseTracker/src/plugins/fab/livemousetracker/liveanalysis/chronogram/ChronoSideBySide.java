...
public class ChronoSideBySide extends ChronoFeatureAbstract implements ProcessAnimalPairFeature {

	public ChronoSideBySide() {
		super ( "Side by side Contact" , "Animals are side by side." , TimeLineDataType.BOOLEAN );
	}

	public void process( Animal animalA , Animal animalB, Integer frameStart , Integer frameEnd )
	{
//		description = animalA.getName() + " and " + animalB.getName() + " are side by side.";
		...
		for ( int t = frameStart ; t < frameEnd ; t++ )
		{
			...
			if ( detectionA != null && detectionB != null )
			{
				...
				if ( scalarProduct >= 0 // Mice are in the same direction.
				...
				}catch( NullPointerException e)
				{
					...
					//System.err.println("chrono side by side. Detection missing front/back.");
				}
			}
		}
	}
