...
public class ChronoBehind extends ChronoFeatureAbstract implements ProcessAnimalPairFeature {

	public ChronoBehind() {
		super ( "Behind" , "Animal A is behind animal B" , TimeLineDataType.BOOLEAN );
	}

	public void process( Animal animalA , Animal animalB, Integer frameStart, Integer frameEnd )
	{
//		description = animalA.getName() +" is behind " + animalB.getName();
		...
		for ( int t = frameStart ; t < frameEnd ; t++ )
		{
			...
			{
				double mouseABodyToMouseBTail;
				double mouseABodyToMouseBHead;
				try{
					mouseABodyToMouseBTail = detectionA.getMassCenter().toPoint2D().distance(
							detectionB.getBackPoint().toPoint2D() );
					mouseABodyToMouseBHead = detectionA.getMassCenter().toPoint2D().distance(
							detectionB.getFrontPoint().toPoint2D() );
				}catch( NullPointerException e )
				{
					// FIXME: add a can't build event.
//					System.err.println("ChronoBehind: Can't compute. Missing front/back point in detection.");
					continue;
				}

				if ( mouseABodyToMouseBTail < mouseABodyToMouseBHead )
				{
