...
/** This version use the triangle call to create a map
 * an transform will keep using that map.
 * */
public class TriangleDrawerFast {

	/** contains the index from source to target transform */
	int [] transformOffset = new int[512*424];

	public void applyTransform( IcyBufferedImage targetImage, IcyBufferedImage sourceImage )
	{
		short[] dataSource = sourceImage.getDataXYAsShort( 0 );
		short[] dataTarget = targetImage.getDataXYAsShort( 0 );

		for ( int i = 0 ; i < 512*424 ; i++ )
		{
			dataTarget[ i ] = dataSource[ transformOffset[i] ];
		}
	}
    ...
    public void drawTexturedLine(int y,
...
//        					Array1DUtil.getValue(srcArray, (v_i * src_pitch) + u_i, sourceImage.isSignedDataType() ));
        			...
        			transformOffset[ offset ] = (v_i * src_pitch) + u_i;
